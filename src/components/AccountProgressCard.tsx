...
export const AccountProgressCard: React.FC<AccountProgressCardProps> = ({
  ...
  return (
    <div className="glass-panel p-5 md:p-6 transition-all relative overflow-hidden border-2 border-[var(--gruv-border)]">
      ...
      {/* FundedNext Futures Drawdown Banner */}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 mb-2 pb-2 border-b border-[var(--gruv-border)]">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-bold text-xs text-[var(--gruv-yellow)] uppercase tracking-wider">
                FundedNext Futures Drawdown Engine
              </span>
            </div>
            <div className="text-xs text-[var(--gruv-muted)]">
              Max Loss Limit: <strong className="text-[var(--gruv-fg)]">${(progress.maxLossLimit || 1384.55).toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs mb-3">
            <div className="bg-[var(--gruv-bg-soft)] p-2.5 rounded-lg border border-[var(--gruv-border)]">
              <div className="text-[var(--gruv-muted)] text-[10px] uppercase">ACCOUNT HARD FLOOR</div>
              <div className="text-sm font-bold text-[var(--gruv-red)]">
                ${(progress.hardAccountFloor || 48527.40).toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </div>
            </div>

            <div className="bg-[var(--gruv-bg-soft)] p-2.5 rounded-lg border border-[var(--gruv-border)]">
              <div className="text-[var(--gruv-muted)] text-[10px] uppercase font-bold">REMAINING DAILY CUSHION</div>
            ...
            <div className="bg-[var(--gruv-bg-soft)] p-2.5 rounded-lg border border-[var(--gruv-border)]">
              <div className="text-[var(--gruv-muted)] text-[10px] uppercase">INTRADAY DRAWDOWN</div>
              <div className="text-sm font-bold text-[var(--gruv-red)]">
                -${(progress.intradayLoss || 207.80).toFixed(2)}
              </div>
            </div>
          </div>
